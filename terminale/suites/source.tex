...
\begin{question}{arithDef}\bareme{\barP}
\AMCnoCompleteMulti
...
\begin{question}{geoDef}\bareme{\barO}
Quelle affirmation est exacte ?
\begin{multicols}{2}\AMCBoxedAnswers
			\begin{reponses}
				\mauvaise{$b_{n+1}=\py{b0}\times b_{n}$}
				\mauvaise{$b_{n+1}=\py{b0}\times \py{q}^n$}
...
\begin{question}{somme1}\bareme{\barQ2}
Calculer $S_{\py{p}}$ à $10^{-2}$ près.
			\begin{reponseshoriz}
				\bonne{$S_{\py{p}} \approx \py{s1}$}
				\mauvaise{$S_{\py{p}} \approx \py{s2}$}
				\mauvaise{$S_{\py{p}} \approx \py{s3}$}
